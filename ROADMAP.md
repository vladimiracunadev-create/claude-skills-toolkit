...
- [x] **`bitcoin-custody-audit`** — auditoría de custodia Bitcoin en 14 etapas, hermano de dominio de `security-audit`: aquel audita el repositorio (CVE, SAST, secretos), este audita la custodia (procedencia de cada semilla cruzada con la matriz de avisos, arquitectura del quorum, aislamiento del nodo, verificabilidad del artefacto, runbook y roles). Su regla central es la honestidad de cobertura: una etapa que no pudo ejecutarse sale **OMITIDA, nunca aprobada**, y un cruce firmante×aviso que no se puede decidir se reporta como indecidible en vez de resolverse como «no afectado». Nunca pide, lee ni acepta material secreto. Portado desde `rootcause-bitcoin-defense` y **desacoplado** de aquel repo: opera sobre `Path.cwd()` con solo stdlib.
- [ ] Tests por skill (no solo estructura) — al menos un happy path por cada uno. *(progreso: 6/15 — `security-audit` con 8 tests de `compute_coverage`, más 31 tests funcionales de los cuatro skills previos y 35 de `bitcoin-custody-audit`)*
